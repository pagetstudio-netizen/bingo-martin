import{a as q,b as E,G as P,r as l,u as T,j as e,L as k,f as j,e as V}from"./index-BmPiDcAZ.js";import{u as x}from"./useQuery-DlIi2Jzu.js";import{u as $}from"./useMutation-DX5WEgip.js";import{g as O}from"./countries-DOninoG6.js";import{T as D}from"./trending-up-BCwCvI1o.js";import{C as K}from"./credit-card-C7-XBXbZ.js";import{C as Q}from"./chevron-right-BgZHlTPy.js";const X="/assets/t%C3%A9l%C3%A9chargement_(80)_1787363581764-DIwCG-l1.png",_="/assets/70df605a42dc9ac4254ef5fd2de61af4_1790106066950-ZA0zkBlV.jpg";function te(){const{user:d,refreshUser:z}=q(),{toast:a}=E(),N=P(),[n,u]=l.useState(""),[t,g]=l.useState(null),[,C]=T(),b=(d?O(d.country):null)?.currency||"XOF",h=b==="FCFA"?"XOF":b,{data:s}=x({queryKey:["/api/settings/withdrawal"],staleTime:0,refetchOnMount:!0}),p=s?.minWithdrawal??1500,w=s?.withdrawalFees??18,m=s?.withdrawalStartHour??9,y=s?.withdrawalEndHour??17,F=n?Math.floor(Number(n)*(1-w/100)):0,v=new Date().getHours(),I=v>=m&&v<y,{data:i=[],isLoading:S}=x({queryKey:["/api/wallets"],refetchOnWindowFocus:!0}),{data:L=[]}=x({queryKey:["/api/user/products"]}),W=L.some(r=>r.status==="active");l.useEffect(()=>{const r=localStorage.getItem("selectedWalletId");if(r&&i.length>0){const o=i.find(M=>M.id===parseInt(r));o&&g(o),localStorage.removeItem("selectedWalletId")}},[i]),l.useEffect(()=>{if(!t&&i.length>0){const r=i.find(o=>o.isDefault);r&&g(r)}},[i,t]);const f=$({mutationFn:async r=>(await V("POST","/api/withdrawals",r)).json(),onSuccess:()=>{a({title:"Demande envoyée",description:"Votre demande de retrait a été envoyée."}),z(),N.invalidateQueries({queryKey:["/api/withdrawals"]}),u("")},onError:r=>{a({title:"Retrait échoué",description:r.message||"Impossible d'enregistrer le retrait.",variant:"destructive"})}}),A=()=>{if(!I){a({title:"Horaires de retrait",description:`Les retraits sont disponibles de ${m}h à ${y}h`,variant:"destructive"});return}if(!W){a({title:"Produit requis",description:"Vous devez avoir un produit actif pour effectuer un retrait",variant:"destructive"});return}if(!n||n<p){a({title:"Montant invalide",description:`Le montant minimum est de ${p} ${h}`,variant:"destructive"});return}if(!t){a({title:"Compte requis",description:"Veuillez sélectionner un compte bancaire",variant:"destructive"});return}f.mutate({amount:Number(n),walletId:t.id})};if(S)return e.jsx("div",{className:"min-h-screen bg-white flex items-center justify-center",children:e.jsx(k,{className:"w-8 h-8 animate-spin text-[#c8102e]"})});if(!d)return null;const R=parseFloat(d?.balance||"0"),H=i.length>0,c=h==="XOF"?"FCFA":h;return e.jsxs("main",{className:"withdraw-reference min-h-screen bg-[#f7f4f2]",children:[e.jsx("style",{children:`
        .withdraw-reference {
          color: #151515;
