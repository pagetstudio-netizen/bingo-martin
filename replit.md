# Bingo - Propreté et protection du linge

## Overview

Bingo est une marque du groupe turc HAYAT DHC, commercialisée dans plus de 44 pays. En Algérie, ses produits sont fabriqués localement à l’usine de Bouinan. La marque place la qualité supérieure, la propreté et la protection des vêtements au cœur de son engagement.
...
- The existing `session` table is preserved when applying the application schema.
- The development database schema is applied from the checked-in migrations before the first run.
- `npm run dev` has been verified to serve the login page and complete database seeding.
